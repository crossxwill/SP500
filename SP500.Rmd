...
A portfolio of `$10,000` at the beginning of the Great Depression never recovers even after 8.6 years. In stark contrast, a portfolio of `$10,000` recovers approximately 4 years after the Financial Crisis.
...
Split the data sets between Great Depression and Financial Crisis. Then add new variable for portfolio age (i.e., the number of days since the initial `$10,000` investment.)

```{r}
GD <- GSPC %>% 
  filter(date >= GDstartdate) %>% 
  mutate(PortfolioAge = row_number() - 1)
...
ggplot(GD_FC2_gather, aes(x=PortfolioAge, y=PortfolioValue)) +
  ...
  scale_x_continuous(name="Portfolio Age (in days)",breaks=seq(0,365*9,365)) +
